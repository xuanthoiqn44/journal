 ...
 *
 */
class EditorLoginForm extends Model
{
    public $email;
    public $password;
    ...
    public function validatePassword($attribute, $params)
    {
        if (!$this->hasErrors()) {
            ...
            if ($user)
            {
                ...
                else if ($user->editors)
                {
                    if ($user->editors->Status_Active==1)
                    {
    ...
    public function getUser()
    {
        if ($this->_user === false) {
            ...
            //$this->_user = User::findOne(['EmailID'=>$this->email]);
            $this->_user = User::find()
                ->where(['EmailID'=>$this->email])
                ->with(['editors'])
                ->limit(1)
                ->one();
        }
